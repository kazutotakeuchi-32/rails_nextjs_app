...
import type { CurrentArticleProps } from '@/types/article'
...
const CurrentArticles: NextPage = () => {
  ...
  const url =
    process.env.NEXT_PUBLIC_API_BASE_URL + '/current/articles?page=' + page

  const { data, error } = useSWR(user.isSignedIn ? url : null, fetcher)
  if (error) return <Error />
  if (!data) return <Loading />

  const articles: CurrentArticleProps[] = camelcaseKeys(data.articles)
  const meta = camelcaseKeys(data.meta)

  const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
    router.push(`/current/articles?page=${value}`)
  }
  ...
  return (
    ...
    >
      <Container maxWidth="md" sx={{ pt: 6, px: 4 }}>
        ...
        {articles.map((article: CurrentArticleProps, i: number) => (
          <>
            <Box
              key={i}
              sx={{
